function [B... BRF
    ...
    ( X, Y, Z_to_remove... height to remove [m]
    , Z_last_removal_dw...
...
elseif strcmp(options.Algorithm, 'FFT')
    [~, Z_removal, Z_residual...
    , T_P, Z_to_remove_dw, Z_removal_dw, Z_residual_dw...
    , Z_to_remove_ca, Z_removal_ca, Z_residual_ca] = DwellTime2D_FFT_Test(Z_to_remove, Z_last_removal_dw, B, dw_range, ca_range);
else
...
if options.isDownSampling == true
    ...
    [C, d, C_T] = DwellTime2D_Assemble_C_d(Nr, Nt, BRF_params, Z_to_remove, X, Y, P, X_BRF, Y_BRF, Z_BRF, ca_range, BRF_mode, viewFullAperture);
